...
def convert_moral_acceptability_text_to_class_wild_v11(targets, preds):
    ...
    coverage = covered_count / len(preds_classes)
    print(covered_count, len(preds_classes), "label coverage rate: %0.3f" % (coverage))
    return target_classes, preds_classes, coverage
...
def convert_moral_acceptability_text_to_class(targets, preds):
    ...
    # for k, v in label_map_count:
    #     print(v, k)
    coverage = (1 - (preds_classes.count(99) / len(preds_classes)))
    print("label coverage rate: %0.3f" % coverage)
